...
def cmd_holidays(message):
    try:
        ...
        # === ЭМОДЗИ ДЛЯ ПРАЗДНИКОВ (подбирается по ключевым словам) ===
        def get_holiday_emoji(holiday_text):
            text_lower = holiday_text.lower()
            
            if any(w in text_lower for w in ['пицц', 'бургер', 'шоколад', 'конфет', 'торт', 'пирог', 'морожен', 'кофе', 'чай', 'пив', 'вин', 'коктейль', 'еда', 'хлеб', 'сыр', 'фрукт', 'яблок', 'лимонад', 'карамель', 'попкорн', 'суши', 'ролл', 'шашлык', 'шаурма', 'блин', 'пельмен', 'вареник', 'чебурек', 'пончик', 'кекс', 'пряник', 'паста', 'макарон', 'десерт', 'сладост', 'мармелад']):
                return '🍔'
            elif any(w in text_lower for w in ['кино', 'фильм', 'театр', 'музык', 'танец', 'песн', 'худож', 'книг', 'поэзи', 'писател', 'актер', 'режиссер', 'искусств', 'библиотек', 'музе', 'архив', 'культур', 'опер', 'балет', 'джаз', 'рок', 'фотограф', 'цирк', 'юмор', 'смех']):
                return '🎨'
            elif any(w in text_lower for w in ['программист', 'учител', 'врач', 'строител', 'воен', 'лётчик', 'космонавт', 'моряк', 'пожарн', 'полиц', 'медсестр', 'стоматолог', 'работников', 'инженер', 'архитектор', 'дизайнер', 'журналист', 'адвокат', 'юрист', 'бухгалтер', 'менеджер', 'продав', 'повар', 'пекарь', 'шахтёр', 'энергетик', 'почтальон', 'водител']):
                return '👔'
            elif any(w in text_lower for w in ['матер', 'отц', 'ребенк', 'детей', 'семь', 'любов', 'валентин', 'бабушк', 'дедушк', 'объятий', 'поцелу', 'друз', 'дружб', 'спасиб', 'доброт', 'счасть', 'радост', 'улыбк', 'комплимент']):
                return '❤️'
            elif any(w in text_lower for w in ['международный', 'всемирный', 'мировой', 'планет', 'земл', 'оон', 'юнеско', 'космос', 'авиаци', 'космонавтик', 'океан', 'экологи', 'природ', 'климат', 'погод', 'вод', 'воздух', 'энерг', 'мир', 'толерантность']):
                return '🌍'
            elif any(w in text_lower for w in ['россии', 'рф', 'отечеств', 'русский', 'народа', 'беларуси', 'украины', 'казахстан', 'армении', 'грузии', 'азербайджан', 'молдов', 'узбекистан', 'китай', 'япон', 'коре', 'инди', 'израил', 'бразил', 'америк', 'сша', 'англ', 'франц', 'герман', 'итал', 'испан']):
                return '🇷🇺'
            elif any(w in text_lower for w in ['пасх', 'рождеств', 'крещен', 'троиц', 'маслениц', 'православн', 'церковн', 'ураза', 'байрам', 'рамадан', 'курбан', 'ханук', 'будд', 'инду', 'мусульм', 'христиан', 'католич', 'святых']):
                return '🙏'
            elif any(w in text_lower for w in ['весеннего', 'осеннего', 'солнцестояния', 'лес', 'гор', 'рек', 'озер', 'море', 'океан', 'животных', 'птиц', 'рыб', 'насекомых', 'растен', 'цвет', 'дерев', 'заповедник', 'парк', 'экологи']):
                return '🌿'
            elif any(w in text_lower for w in ['шутк', 'прикол', 'розыгрыш', 'смех', 'весель', 'фестиваль', 'карнавал', 'вечеринк', 'танц', 'салют', 'фейерверк', 'концерт', 'шоу', 'парад']):
                return '🎉'
            elif any(w in text_lower for w in ['спорт', 'теннис', 'футбол', 'баскетбол', 'хоккей', 'волейбол', 'шахмат', 'олимп']):
                return '⚽'
            elif any(w in text_lower for w in ['наук', 'физик', 'хими', 'биолог', 'математ', 'изобретен', 'открыт']):
                return '🔬'
            elif any(w in text_lower for w in ['здоров', 'медиц', 'врач', 'больниц', 'аптек', 'фармац']):
                return '💊'
            elif any(w in text_lower for w in ['интернет', 'компьютер', 'сайт', 'веб', 'цифр', 'айти', 'соцсет']):
                return '💻'
            else:
                return '📌'
